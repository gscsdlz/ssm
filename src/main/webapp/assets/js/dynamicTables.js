;(function($) {
    $.fn.dynamicTables = function(options) {
        ...
        function initTableData() {
            ...
            //键值映射转换
            if (settings.dataKeys.length !== 0) {
                var newData = [];
                for (var i = 0; i < settings.data.length; i++) {
                    var data = [];
                    for (var j = 0; j < settings.dataKeys.length; j++) {
                        data.push(settings.data[i][settings.dataKeys[j]])
                    }
                    newData.push(data)
                }
            ...
            $.post(settings.addURL, {info:info}, function(response){
                if(response.status == true) {
                    ...
                    for(var i = 0; i < response.errors.length; i++) {
                        $("#addForm").children().eq(response.errors[i].index).children().eq(1).append('<p class="text-danger">' + response.errors[i].info + '</p>');
                    }
                }
            })
        })
